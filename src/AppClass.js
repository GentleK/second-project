...
import './page/styles/RockPaperScissors.css';
import BoxClass from './page/components/BoxClass';
...
export default class AppClass extends Component {
    ...
    play = (userChoice)=>{
        ...
        this.setState({ userSelect : this.choice[userChoice], computerSelect : this.choice[computerChoice] });
        this.judgement(userChoice, computerChoice);
    }
    ...
    judgement = (userChoice, computerChoice)=>{
        if( userChoice == computerChoice ){
            this.setState({ userResult : "TIE", computerResult : "TIE", userBoxClass : "box", computerBoxClass : "box"});
        }else if( 
            ( "rock" == userChoice && "scissors" == computerChoice ) ||
            ( "paper" == userChoice && "rock" == computerChoice ) ||
            ( "scissors" == userChoice && "paper" == computerChoice )
            ){
                this.setState({ userResult : "WIN", computerResult : "LOSE", userBoxClass : "winBox", computerBoxClass : "loseBox"});
        }else{
            this.setState({ userResult : "LOSE", computerResult : "WIN", userBoxClass : "loseBox", computerBoxClass : "winBox"});
        }
    }

    componentDidMount(){
        //UI 로드가 종료되면(render() 이후) 호출 : API콜을 해주는등의 작업이 보통 들어감..
        console.log("componentDidMount");

    }

    componentDidUpdate(){
        //props가 전달되거나 setState()가 호출되거나 등등의 render가 다시호출되는 작업이 종료되었을때 호출..
        console.log("componentDidUpdate");
    }

    componentWillUnmount(){
        //현재의 컴퍼넌트가 사용종료될때 바로 직전에 호출..
    }
    ...
    render() {
        ...
        return (
            <div>
                ...
                <div>
                    <div className='main'>
                        <BoxClass title='You' item={this.state.userSelect} result={this.state.userResult} resultClass={this.state.userBoxClass} />
                        <BoxClass title='Computer' item={this.state.computerSelect} result={this.state.computerResult} resultClass={this.state.computerBoxClass} />
                    </div>
                    <div className='main'>
                        <button className='game-button' onClick={()=>this.play("scissors")}><img className='button-image' src={this.choice.scissors.img} /></button>
                        <button className='game-button' onClick={()=>this.play("rock")}><img className='button-image' src={this.choice.rock.img} /></button>
                        <button className='game-button' onClick={()=>this.play("paper")}><img className='button-image' src={this.choice.paper.img} /></button>
                    </div>
                </div>
            </div>
        )
