...
const HMShopingMallNavbar = ({setHMShopingMallAuthenticate, HMShopingMallAuthenticate}) => {
    const menuList = ['여성', 'Divided', '남성', '신생아/유아', '아동', 'H&M HOME', 'Sale', '지속가능성'];
    let [width, setWidth] = useState(0);
    const navigate = useNavigate();
    const goToLoginout = ()=> {
        if( HMShopingMallAuthenticate ){
            setHMShopingMallAuthenticate(false);
        }else{
            navigate('/HMShopingMall/login');
        }
    }
    ...
    function search(event) {
        if(event.key === 'Enter' ){
            ...
            navigate(`/HMShopingMall/?q=${keyword}`);
        }
    }
...
export default HMShopingMallNavbar
